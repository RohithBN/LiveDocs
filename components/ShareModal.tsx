  ...
  const ShareModal = ({ roomId, collaborators, creatorId, currentUserType }: ShareDocumentDialogProps) => {
    ...
    const shareDocumentHandler = async () => {
      setLoading(true);
       ...
       const done=await updateDocumentAccess({ 
        roomId, 
        email, 
    ...
    return (
      <Dialog open={open} onOpenChange={setOpen}>
        {/* This button should not be a button element to prevent button nesting */}
        <DialogTrigger>
          <div className="gradient-blue flex h-9 gap-1 px-4 cursor-pointer rounded-lg py-1">
            <Image
